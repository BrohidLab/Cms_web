...
<div class="p-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Perbaiki artikel untuk website</p>
    </div>
    ...
    <form action="{{ route('article.update', $article->id) }}" 
        ...
        <div>
            ...
            <textarea name="content"
                      id="editor"
                      rows="10"
                      class="w-full border rounded-lg px-4 py-2 focus:ring focus:ring-blue-200 focus:outline-none">{{ old('content', $article->content) }}</textarea>
            @error('content')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
